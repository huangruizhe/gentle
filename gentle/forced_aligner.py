...
from gentle.transcriber import SingleThreadedTranscriber
...
class ForcedAligner():
    ...
    def __init__(self, resources, transcript, nthreads=4, **kwargs):
        ...
        self.kaldi = standard_kaldi.Kaldi(
            resources.nnet_gpu_path,
            gen_hclg_filename,
            resources.proto_langdir
        )
        self.stt = SingleThreadedTranscriber(self.kaldi)

    def transcribe(self, wavfile, progress_cb=None, logging=None):
        words, duration = self.stt.transcribe(wavfile, progress_cb=progress_cb)
