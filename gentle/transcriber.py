...
class SingleThreadedTranscriber:
    def __init__(self, kaldi, chunk_len=20, overlap_t=2):
        self.chunk_len = chunk_len
        self.overlap_t = overlap_t
        self.kaldi = kaldi
    ...
    def transcribe(self, wavfile, progress_cb=None):
        ...
        def transcribe_chunk(idx):
            ...
            if len(buf) < 4000:
                ...
                self.kaldi.push_chunk(buf)
                ret = self.kaldi.get_final()
                # k.reset() (no longer needed)
                # self.kaldi.stop()
