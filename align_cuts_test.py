...
for icut, cut in enumerate(cuts):
    ...
    offset1 = max(0, cut.start + cut.supervisions[0].start - args.margin)
    offset2 = cut.start + cut.supervisions[0].start - args.margin
    duration = cut.supervisions[0].duration + 2 * args.margin
    
    with gentle.resampled(audiofile, offset=offset1, duration=duration) as wavfile1, \
        gentle.resampled(audiofile, offset=offset2, duration=duration) as wavfile2:
        assert wavfile1.shape == wavfile2.shape, f"{wavfile1.shape} vs {wavfile2.shape}"
